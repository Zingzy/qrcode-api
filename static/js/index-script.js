...
function generateQRCode() {
    ...
    var qrType = document.getElementById("qr-type").value;
    var fillColor = encodeURIComponent(document.getElementById("fill-color-input").value || "#000000");
    var bgColor = encodeURIComponent(document.getElementById("background-color-input").value || "#ffffff");
    var gradientColor = encodeURIComponent(getRgbFormat(document.getElementById("gradient-color-input").value || "#6a1a4c"));
    var gradient2Color = encodeURIComponent(getRgbFormat(document.getElementById("gradient2-color-input").value || "#40353c"));

    var apiUrl;

    if (qrType === "simple") {
        apiUrl = `https://jinxedqrcode.forcc1.repl.co/simple?url=${encodeURIComponent(userInput)}&fill=${fillColor}&back=${bgColor}`;
    } else {
        apiUrl = `https://jinxedqrcode.forcc1.repl.co/radial?url=${encodeURIComponent(userInput)}&gradient1=${gradientColor}&gradient2=${gradient2Color}`;
    }
